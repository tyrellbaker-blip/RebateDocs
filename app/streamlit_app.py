...
with st.sidebar:
    ...
    st.markdown(
        "1. Upload one or more PDF bulletins using the uploader above.\n"
        "2. Each file is read and temporarily saved so the program can work with it.\n"
        "3. The system scans the text line by line, looking for program details "
        "(like Program ID, dates) and any rebates listed with dollar amounts.\n"
        "4. Results are cleaned up so duplicates are removed.\n"
        "5. You’ll see a table of unique rebates for each document, and a combined "
        "table if you upload multiple files.\n\n"
        "**The goal:** Instead of flipping through pages of PDFs, you get one clean list "
        "with the program, dates, models/years, amounts, and any exclusions."
    )
    st.divider()
    st.markdown("**Tip:** If something looks missing, open the Debug section to check the raw text the system saw.")
